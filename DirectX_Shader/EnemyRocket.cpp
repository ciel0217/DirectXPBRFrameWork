...
void EnemyRocket::Init()
{
	...
	m_Audio->Load("asset/music/rockon.wav");
	m_Audio->Play(true);
	...
	m_Alert = ManagerScene::GetInstance()->GetCurrentScene()->GetGameObject<MissileAlert>(e2DObject);
}

void EnemyRocket::Uninit()
{
	m_Audio->StopAll();
	delete m_Audio;
}
...
void EnemyRocket::Update()
{
	if (!m_IsPause) {
		...
		if (m_FrameCount % 20 == 0) {
			if (m_TargetObject && CanTracking() && !m_LostTarget) {
				//âÒì]å„ÇÃÉxÉNÉgÉã
				D3DXVECTOR3 dir = m_TargetObject->GetPosition() - m_Position;
				D3DXVec3Normalize(&dir, &dir);
				...
				if (degree > 5.0f) {
					m_Rotation = RotateAxis(axisx, 5.0f, m_Rotation);
				}
				else {
					m_Rotation = FromToRotation(UnitX, dir);
				}
...
void EnemyRocket::OnCollisionEnter(CGameObject3D * collider)
{
	m_IsDestroy = true;
	m_Alert->SetActive(false);
	m_ParticleEmitter->SetDestroy(m_IsDestroy);
